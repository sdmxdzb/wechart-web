...
<body>
    ...
    <div id="mainLayout" class="easyui-layout hidden" data-options="fit: true">
        ...
        <div data-options="region: 'west', title: '菜单导航栏', iconCls: 'icon-standard-map', split: true, minWidth: 200, maxWidth: 400" style="width: 220px; padding: 1px;">
			...
			{{each data as p_menu}}
				{{if (p_menu.parentId==null)}}
   				 <div title="{{p_menu.name }}" style="padding: 5px;" data-options="border:false,iconCls:'{{p_menu.icon }}'">
					<div>
					{{each data as c_menu}}	
						{{if (c_menu.parentId==p_menu.id)}}
						<a id="btn" class="easyui-linkbutton" data-options="plain:true,iconCls:'{{c_menu.icon }}'" style="width:98%;margin-bottom:5px;" onclick="window.mainpage.mainTabs.addModule('{{c_menu.name}}','{{c_menu.url }}','{{c_menu.icon }}')">{{c_menu.name}}</a>
						{{/if}}	
					{{/each}}
					</div>
				</div>
